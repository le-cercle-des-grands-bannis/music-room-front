...
const AlertMessage = ({message, severity, handleClose}: {message: string | null; severity: AlertColor; handleClose: () => void}): JSX.Element | null => {
    if (message) {
        return (
            <AlertWithAction severity={severity} action={
                <CancelIconOnAction onClick={handleClose} />
            }>
                {message}
            </AlertWithAction>
        );
...
const getMessageByLang = (payload: string | null, error: string | null): string | null => payload || error;
...
export const Notification = (): JSX.Element => {
    ...
    const handleClose = useCallback((event: any, reason: SnackbarCloseReason) => {
        ...
        if (event && "clickaway" === reason && 0 < event.screenX && 0 < event.screenY && 0 < event.clientX && 0 < event.clientY) {
            dispatch(openCloseNotif(false));
        }
    ...
    useEffect(() => {
        try {
            if (cookies.success_message) {
                const payload: Base64Cookie = JSON.parse(Buffer.from(cookies.success_message, 'base64').toString('ascii'));
                dispatch(fetchPayload(payload.message))
            }
            if (cookies.error_message) {
                const payload: Base64Cookie = JSON.parse(Buffer.from(cookies.error_message, 'base64').toString('ascii'));
    ...
    return (
        ...
        >
            <div>
                {payload && !error && <AlertMessage message={getMessageByLang(payload, error)} severity={"success"} handleClose={handleCloseWithButton}/>}
                {error && !payload && <AlertMessage message={getMessageByLang(payload, error)} severity={"error"} handleClose={handleCloseWithButton}/>}
            </div>
        </Snackbar>
    );
};
